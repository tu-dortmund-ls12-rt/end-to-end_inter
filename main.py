...
import utilities.analysis_becker as ana_becker
...
def flatten3(ce_ts_sched):
    ...
    ce_ts_sched_flat = [
        (ce, ts, sched) for ce_lst, ts, sched in ce_ts_sched for ce in ce_lst
    ]
...
def our_mrt_mRda(lst, bcet):
    ...
    if bcet != 0:  # the dispatcher can only handle execution != 0
        rat_sched = schedule_task_set(
            ce_lst, rat_ts, print_status=False
        )  # schedule with bcet
    else:
    ...
    mrt_res = [a_our.max_reac_local(ce, ts, sched, rat_ts, rat_sched) for ce in ce_lst]
    mRda_res = [a_our.max_age_local(ce, ts, sched, rat_ts, rat_sched) for ce in ce_lst]
    ...
    for et in schedules_todo:
        ...
        if et != 0:  # the dispatcher can only handle execution != 0
            sched_et = schedule_task_set(
                ce_lst, ts_et, print_status=False
...
if __name__ == "__main__":
    ...
    for opt, arg in opts:
        if opt in ("-s", "--switch"):  # define which part of the code is being executed
            ...
            "--bench",
            "--benchmark",
        ):  # benchmark 'waters' or 'uunifast'
            benchmark = str(arg)
        elif opt in (
            "-n",
            "--number",
        ):  # number of tasksets or of interconnected chains for the generations
            number = int(arg)
        elif opt in (
            "-p",
            "--proc",
            "--processors",
        ):  # number of processors that are used for the computations
            processors = int(arg)
        else:
            breakpoint()
    ...
    if code_switch == 1:
        ...
        write_data(
            f"output/1generation/single_ce_ts_{util=}_{number=}_{benchmark=}.pkl", ce_ts
        )
    ...
    # == Single ECU Experiment ==
        ...
        ce_ts = load_data(
            f"output/1generation/single_ce_ts_{util=}_{number=}_{benchmark=}.pkl"
        )
        ...
        with Pool(processors) as p:
            res_our = p.starmap(
                our_mrt_mRda_lst, zip(ce_ts, itertools.repeat(bcet_ratios))
            )
        ...
        write_data(
            f"output/2results/single_ce_ts_{util=}_{number=}_{benchmark=}.pkl", ce_ts
        )
    ...
    if code_switch == 3:
        ...
        for j in range(0, number_interconn_ce_chains):
            com_tasks = comm.generate_communication_taskset(
                20, 10, 1000, True
            )  # generate communication tasks
            com_tasks = list(
                np.random.choice(com_tasks, 4, replace=False)
            )  # randomly choose 4
            choice_ces = np.random.choice(len(single_ce_ts_flat), 5, replace=False)
            ces = [
                single_ce_ts_flat[entry][0] for entry in choice_ces
            ]  # randomly choose 5
            inter_chains.append(
                [
                    ces[0],
    ...
    if code_switch == 4:
        ...
        for bcet in bcet_ratios:
            with Pool(processors) as p:
                res_our_mrda[bcet] = p.starmap(
                    cutting_thm_mrda, zip(inter_chains, itertools.repeat(bcet))
                )
                res_our_mrt[bcet] = p.starmap(
                    cutting_thm_mrt, zip(inter_chains, itertools.repeat(bcet))
                )
        ...
        results_mrt = {"D07": res_davare, "D19": res_duerr_mrt, "Our": res_our_mrt}
    ...
    if code_switch == 5:
        ...
        for ana_name, attr_name, item_name in extract_lst_mrda:
            if item_name is not None:
                results_mrda[ana_name] = [
                    getattr(ce, attr_name)[item_name] for ce, _ in single_ce_ts_flat
                ]
            else:
                results_mrda[ana_name] = [
                    getattr(ce, attr_name) for ce, _ in single_ce_ts_flat
                ]
        ...
        plot.plot_reduction(
            results_mrda, "D07", f"output/3plots/single_red_mrda_{benchmark=}.pdf"
        )
        plot.plot_reduction(
            results_mrt, "D07", f"output/3plots/single_red_mrt_{benchmark=}.pdf"
        )

        plot.plot_gap_reduction(
            results_mrda,
            "D07",
            "1.0",
            f"output/3plots/single_gap_mrda_{benchmark=}.pdf",
        )
        plot.plot_gap_reduction(
            results_mrt, "D07", "1.0", f"output/3plots/single_gap_mrt_{benchmark=}.pdf"
        )
    ...
    if code_switch == 6:
        ...
        check_or_make_directory("output/3plots")
        plot.plot_reduction(
            results_mrda, "D07", f"output/3plots/inter_red_mrda_{benchmark=}.pdf"
        )
        plot.plot_reduction(
            results_mrt, "D07", f"output/3plots/inter_red_mrt_{benchmark=}.pdf"
        )
